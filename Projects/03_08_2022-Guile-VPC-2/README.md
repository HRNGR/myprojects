...
- discuss about how to connect to the "hrn-az1b-private-subnet" instance
...
```bash
eval "$(ssh-agent)"    Dikkat
```
-  Add the your private ssh key to the ssh-agent.

```bash
ssh-add ./[your pem file name]    Dikkat
```
...
- Select "hrn-private-rt" ---> Routes ----> Edit Rule ---> Add Route
```
Destination     : 0.0.0.0/0
Target ----> Nat Gateway ----> hrn-nat-gateway
```
- click save routes
...
- Select hrn-nat-gateway --- > Actions ---> delete Nat Gateway
...
- Go to Route Table and select "hrn-private-rt"
