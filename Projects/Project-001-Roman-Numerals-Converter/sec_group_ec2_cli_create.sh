...
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t2.micro --key-name FirstKey --security-groups roman_numbers_sec_grp --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=roman_numbers}]' --user-data /home/ec2-user/userdata.sh
...
instance silinmeden securty group silinmiyor
